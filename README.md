...
一个面向 **grok** 的极简 OAuth2 反向代理。客户端只用一个固定的 `client_api_key`,
...
config.json 只有 4 个字段。零第三方依赖(纯 Go 标准库)。
...
|------|------|------|
| `listen` | 监听地址 | `:8080` |
| `client_api_key` | 客户端访问本代理的固定 key(空则不校验) | — |
| `name` | 路由前缀 `/name/...` + 凭证文件名 | `grok` |
| `cred_dir` | 凭证目录,凭证存 `<cred_dir>/<name>.json` | `./data` |
...
命令:`grok-proxy [config.json] [login]` —— 带 `login` 时强制对所有账号重新登录后退出。
...
1. 准备目录,放入 `config.json`(见上,`cred_dir` 为 `/data`)和 `docker-compose.yml`;
2. 启动:
...
3. **首次登录**(容器内无浏览器,看日志拿授权地址):
   ...
   # 在任意浏览器打开该地址完成授权,容器自动写入 ./data/grok.json
   ```

   授权成功后凭证持久化在 `./data/`,之后重启容器自动复用并后台刷新
   (除非 refresh_token 被吊销 —— 删掉 `./data/grok.json` 重启重新登录即可)。

4. 调用:`http://<NAS_IP>:5001/grok/v1/chat/completions`(带 `Authorization: Bearer <client_api_key>`)。
...
凭证格式完全兼容:把旧的 `grok-auth.json` 改名成 `<cred_dir>/grok.json` 即可,
`config.json` 里删掉不用的字段(upstreams、token_url、client_id 等)。
